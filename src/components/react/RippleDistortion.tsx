/**
 * Ondulation au pointeur — port du composant « Ripple Distortion » de React Bits
 * (https://reactbits.dev/animations/ripple-distortion), moteur ogl.
 * Shaders, champ de déplacement instancié (MAX_WAVES), composition avec dispersion,
 * glint et teinte : repris tels quels.
 *
 * Adaptations LOOKi :
 *  - l'image reste servie en HTML (enfants du composant) ; le canvas ne s'affiche
 *    qu'une fois la texture chargée, par-dessus, avec le même cadrage (cover centré) ;
 *  - rien n'est rendu sans WebGL, en mouvement réduit, sans pointeur fin ou en économie
 *    de données ; la boucle est mise en pause hors écran et onglet masqué ;
 *  - palette : pas de noir et blanc, teinte turquoise et reflet blanc givré par défaut ;
 *  - dpr plafonné à 1.5.
 */
...
export interface RippleDistortionProps {
  /** URL de l'image (déjà recadrée au format du cadre, pour coïncider avec l'image servie). */
  src: string;
  brushSize?: number;
  strength?: number;
  swirl?: number;
  ...
  quality?: keyof typeof QUALITY_SCALE;
  enabled?: boolean;
  className?: string;
  style?: CSSProperties;
  children?: ReactNode;
...
export default function RippleDistortion({
  ...
  const uniformsRef = useRef<{ wave: { uRings: { value: number } }; composite: Record<string, { value: unknown }> } | null>(null);
  ...
  useEffect(() => {
    ...
    setActive(enabled && !reduce && fine && nav.connection?.saveData !== true && !!gl);
  }, [enabled]);
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([entry]) => { visible = entry.isIntersecting; visible ? start() : stop(); }, { threshold: 0.05 });
    io.observe(mount);
  ...
  return (
    <div className={`ripple-distortion ${className}`.trim()} style={style}>
      {children}
      <div ref={mountRef} className={`ripple-distortion__gl${ready ? ' is-ready' : ''}`} aria-hidden="true" />
    </div>
  );
}
